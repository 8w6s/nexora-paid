...
import { api, fmtUsd, isAccessDenied, goTo404, type OrderDetail } from "../lib/api";
...
export const OrderDetailView: React.FC<{ orderId: string }> = ({ orderId }) => {
  ...
  useEffect(() => {
    api.get<OrderDetail>(`/api/orders/${orderId}`).then((d) => { setO(d); setState("ok"); }).catch((e) => {
  ...
  const wrap = (inner: React.ReactNode) => <main className="container od-page">{inner}<Styles /></main>;
  if (state === "loading") return wrap(
    <div className="od card">
      <div className="od-head" style={{ alignItems: "center" }}><Sk w={140} h={24} /><Sk w={70} h={22} r={100} /></div>
      <Sk h={16} style={{ margin: "8px 0" }} /><Sk h={16} w="60%" />
  ...
  return wrap(
    <div className="od card">
      <div className="od-head"><div><span className="muted">Order</span><h1>{o!.id}</h1></div><span className={`badge ${o!.status}`}>{o!.status}</span></div>
      <div className="od-items">{o!.items.map((i, k) => <div key={k} className="od-line"><span>{i.name} ×{i.quantity}</span><span className="price">{fmtUsd(i.priceUsd * i.quantity)}</span></div>)}</div>
      <div className="od-total"><span>Total</span><span className="price">{fmtUsd(o!.totalUsd)} · {o!.ltcAmount} LTC</span></div>
      {done && (
        <div className="od-keys">
          <div className="ok"><Icon name="check" size={18} /> Delivered keys</div>
          <ul>{o!.deliveredKeys.map((k, i) => <li key={i}><code>{k.code}</code></li>)}</ul>
          <p className="hint">Keep these private — anyone with the code can redeem it.</p>
        </div>
      )}
      {payable && <a className="btn" href={`/checkout?id=${o!.id}`} style={{ justifyContent: "center" }}>Complete payment</a>}
    </div>
  );
};
