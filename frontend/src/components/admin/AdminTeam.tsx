...
const ROLE_COLORS: Record<TeamRole, string> = { admin: "badge-red", manager: "badge-blue", support: "badge-green", viewer: "badge-gray" };
...
export const AdminTeam: React.FC = () => {
  ...
  const load = () => {
    setLoading(true);
    api.get<TeamMember[]>("/api/admin/team").catch(() => [] as TeamMember[]).then(setMembers).finally(() => setLoading(false));
  };
  ...
  const invite = async () => {
    ...
    try {
      ...
      setEmail(""); setInviting(false); load();
    } catch (e) { setErr(e instanceof Error ? e.message : "Failed to send invitation"); }
    finally { setBusy(false); }
  };

  const remove = async (id: string, memberEmail: string) => {
    if (!confirm(`Remove ${memberEmail} from your team?`)) return;
    await api.delete(`/api/admin/team/${id}`).catch(() => {});
    load();
  };
  ...
  return (
    <div className="adm-section">
      <div className="adm-sec-head">
        ...
        <button className="btn" onClick={() => { setInviting(true); setErr(null); setSuccess(null); }}>
      ...
      {success && (
        <div className="team-success">
          ...
          <span>{success}</span>
          <button type="button" className="btn btn-ghost btn-sm" onClick={() => setSuccess(null)}><Icon name="close" size={13} /></button>
        </div>
      )}
      ...
      {inviting && (
        <div className="card" style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <label className="pe-field-label">Email Address
            <input className="input" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="colleague@example.com" onKeyDown={e => e.key === "Enter" && invite()} />
          </label>
          <label className="pe-field-label">Role</label>
          <div className="team-roles">
            {ROLES.map(r => (
              <button key={r.key} type="button" className={`team-role-card ${role === r.key ? "on" : ""}`} onClick={() => setRole(r.key)}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <button className="btn btn-ghost" onClick={() => setInviting(false)}>Cancel</button>
            <button className="btn" onClick={invite} disabled={busy}>
              {busy ? <><Icon name="spinner" size={14} className="is-spinning" /> Sending…</> : "Send Invitation"}
            </button>
          </div>
        </div>
      )}
      ...
      {loading ? <div className="adm-loading"><Icon name="spinner" size={24} className="is-spinning" /></div>
        : members.length === 0
          ? <EmptyState icon="users" title="No Team Members" message="Invite colleagues to help manage your store." action={{ label: "Invite Member", onClick: () => setInviting(true) }} />
          : (
                ...
                <tbody>
                  {members.map(m => (
                    <tr key={m.id}>
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: ".82rem", color: "var(--ink-soft)" }}>{m.email}</div>
                          </div>
                        </div>
                      </td>
                      <td>
                      ...
                      <td>
                        <button className="btn btn-ghost btn-sm btn-danger-icon" onClick={() => remove(m.id, m.email)}><Icon name="close" size={13} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
