      ...
      <div className="grp-editor">
        ...
        <div className="grp-form card">
          <div className="grp-section-title">
            <Icon name="folder" size={16} /> General
          </div>
          ...
          <div className="grp-section-title" style={{ marginTop: 8 }}>
            <Icon name="tag" size={16} /> Badge <span className="badge-opt">(optional)</span>
          </div>
          ...
          <div className="grp-section-title" style={{ marginTop: 8 }}>
            <Icon name="box" size={16} /> Products
          </div>
          ...
          <div className="grp-products-grid">
            {products.map((p) => (
              <label
                key={p.id}
                className={`grp-product-card ${form.productIds.includes(p.id) ? "on" : ""}`}
              >
                <input
                  type="checkbox"
                ...
                {form.productIds.includes(p.id) && (
                  <span className="grp-product-check">
    ...
    <div className="adm-section">
      ...
      {loading ? (
        ...
        <EmptyState
          icon="folder"
          title="No Groups Yet"
          message="Create a group to bundle products under one card."
          action={{ label: "Create Group", onClick: openCreate }}
        />
      ) : (
            ...
            <div key={g.id} className="grp-row card">
              ...
              <div className="grp-row-info">
                ...
                {g.badge && (
                  <span
                    className="grp-badge"
                    style={{ background: g.badgeColor || "var(--brand)" }}
                  >
                    {g.badge}
                  </span>
                )}
                <span className="muted" style={{ fontSize: ".82rem" }}>
                  {g.productIds.length} products · {g.active ? "Public" : "Hidden"}
                </span>
              </div>
