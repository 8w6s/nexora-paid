...
export const AdminBundleOffers: React.FC = () => {
  ...
  const openEdit = (o: BundleOffer) => {
    setForm({
      ...
      endDate: o.endDate ?? "",
      maxUsesPerCart: o.maxUsesPerCart,
      priority: o.priority,
    });
    setEditing(o);
    setCreating(true);
  ...
  const addBundleProduct = (productId: string) => {
    ...
    setForm((f) => ({
      ...f,
      products: [...f.products, { productId, productName: product?.name, qty: 1 }],
    }));
  };

  const removeBundleProduct = (idx: number) =>
    setForm((f) => ({ ...f, products: f.products.filter((_, i) => i !== idx) }));
  const updateQty = (idx: number, qty: number) =>
    setForm((f) => ({ ...f, products: f.products.map((p, i) => (i === idx ? { ...p, qty } : p)) }));
      ...
      <div className="adm-section">
        <div className="adm-sec-head">
          <div>
            ...
            <p className="muted">
              Fill in the details below to {editing ? "edit this" : "create a new"} bundle offer.
            </p>
          </div>
        ...
        >
          ...
          <div className="grid-2">
            ...
            <label className="pe-field-label">
              Discount {form.discountType === "percentage" ? "%" : "Amount"}
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <NumberInput
                  min={0}
                  max={form.discountType === "percentage" ? 100 : undefined}
                  decimal={form.discountType === "fixed"}
                  value={String(form.discountValue)}
                  onChange={(v) => setForm((f) => ({ ...f, discountValue: Number(v) || 0 }))}
                />
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    {[5, 10, 15, 25, 50].map((n) => (
                      <button
                        ...
                        onClick={() => setForm((f) => ({ ...f, discountValue: n }))}
                      >
                        {n}%
                      </button>
                    ))}
          ...
          <p className="pe-field-sub">
            Add the products that must be in the cart to trigger this bundle. Set required quantity
            for each.
          </p>
            ...
            <div style={{ flex: 1 }}>
              <Dropdown<string>
                ...
                options={[
                  ...
                  ...products
                    .filter((p) => !form.products.some((fp) => fp.productId === p.id))
                ...
                <div key={p.productId} className="bundle-product-row">
                  ...
                  >
                    ...
                    <NumberInput
                      min={1}
                      value={String(p.qty)}
                      onChange={(v) => updateQty(i, Number(v) || 1)}
                    />
                  </label>
                  <button
                    type="button"
                    className="btn btn-ghost btn-sm btn-danger-icon"
                    onClick={() => removeBundleProduct(i)}
                  >
    ...
    <div className="adm-section">
      <div className="adm-sec-head">
        ...
        <button className="btn" onClick={openCreate}>
          <Icon name="plus" size={14} /> Create Bundle Offer
        </button>
      </div>
      ...
      <div className="adm-info-card card">
        ...
        <div>
          ...
          <p className="muted" style={{ fontSize: ".84rem", marginTop: 4 }}>
            Bundle Offers automatically apply a discount when a customer's cart contains a specific
            combination of products and quantities. No coupon code required.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message="Create bundle offers to automatically discount product combinations."
          action={{ label: "Create Bundle Offer", onClick: openCreate }}
        />
      ) : (
            ...
            <tbody>
              {offers.map((o) => (
                <tr key={o.id}>
                  ...
                  <td>
                    <span
                      className={`badge ${o.status === "enabled" ? "badge-green" : "badge-gray"}`}
                    >
                      {o.status}
                    </span>
                  </td>
