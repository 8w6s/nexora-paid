...
interface AdminReview {
  id: string; rating: number; body: string; email: string;
  hidden: boolean; createdAt: number; productId: string; productName: string | null;
}
...
export const AdminReviews: React.FC = () => {
  ...
  const load = () => api.get<AdminReview[]>("/api/admin/reviews").then(setList).catch(() => {});
  useEffect(() => { load(); }, []);
  ...
  const toggleHidden = async (r: AdminReview) => {
    ...
    try {
      ...
      setList((prev) => prev?.map((x) => (x.id === r.id ? { ...x, hidden: !r.hidden } : x)) ?? null);
  ...
  return (
    <div className="rvm">
      <SkeletonStyles />
      <p className="intro">Verified-purchase reviews. Hide abusive ones (kept but not shown on the storefront) or delete permanently.</p>
      <div className="card table-card">
        <div className="table-wrap">
          <table>
            <thead><tr><th>Product</th><th>Rating</th><th>Review</th><th>By</th><th>Status</th><th></th></tr></thead>
            <tbody>
              {!list ? Array.from({ length: 4 }).map((_, i) => (
                <tr key={i}><td><Sk w={120} h={13} /></td><td><Sk w={70} h={13} /></td><td><Sk w={200} h={13} /></td><td><Sk w={90} h={13} /></td><td><Sk w={56} h={20} r={100} /></td><td><Sk w={80} h={13} /></td></tr>
              )) : list.length === 0 ? (
                <tr><td colSpan={6} className="empty">No reviews yet.</td></tr>
              ) : list.map((r) => (
                <tr key={r.id} className={r.hidden ? "hidden-row" : ""}>
                  <td className="prod">{r.productName ?? "—"}</td>
                  <td className="stars">{"★".repeat(r.rating)}<span className="dim">{"★".repeat(5 - r.rating)}</span></td>
                  <td className="body">{r.body || <span className="muted">(no text)</span>}</td>
                  <td className="muted sm">{r.email}</td>
                  <td><span className={`badge ${r.hidden ? "hidden" : "visible"}`}>{r.hidden ? "Hidden" : "Visible"}</span></td>
                  <td className="actions">
                    <button className="lnk" disabled={busy === r.id} onClick={() => toggleHidden(r)}>{r.hidden ? "Unhide" : "Hide"}</button>
                    <button className="lnk del" disabled={busy === r.id} onClick={() => remove(r)} aria-label="Delete review"><Icon name="trash" size={14} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
