...
const EMPTY: Omit<QuantityDeal, "id" | "productName"> = {
  name: "", productId: "", variantId: "", buyQty: 2, discountedQty: 1, discountPercent: 50,
  status: "enabled", startDate: "", endDate: "", maxUsesPerCart: undefined, priority: undefined,
};
...
export const AdminQuantityDeals: React.FC = () => {
  ...
  const load = () => {
    ...
    ]).then(([d, p]) => { setDeals(d); setProducts(p); }).finally(() => setLoading(false));
  };
  ...
  const openEdit = (d: QuantityDeal) => {
    setForm({ name: d.name, productId: d.productId, variantId: d.variantId ?? "", buyQty: d.buyQty, discountedQty: d.discountedQty, discountPercent: d.discountPercent, status: d.status, startDate: d.startDate ?? "", endDate: d.endDate ?? "", maxUsesPerCart: d.maxUsesPerCart, priority: d.priority });
    setEditing(d); setCreating(true); setErr(null);
  };
  ...
  const remove = async (id: string) => {
    ...
    await api.delete(`/api/admin/quantity-deals/${id}`).catch(() => {});
    load();
  };
    ...
    <div className="adm-section">
      <div className="adm-sec-head">
        <div>
          ...
          <p className="muted">Fill in the details below to {editing ? "edit this" : "create a new"} quantity deal.</p>
        </div>
      ...
      <div className="card" style={{ padding: "24px", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <label className="pe-field-label">Name
          <input className="input" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Deal name" />
        </label>

        <label className="pe-field-label">Product
          <Dropdown<string>
        ...
        <div className="grid-3">
          <label className="pe-field-label">Buy Quantity
            <span className="pe-field-sub">Customer must add at least this many items.</span>
            <NumberInput min={1} value={String(form.buyQty)} onChange={v => setForm(f => ({ ...f, buyQty: Number(v) || 1 }))} />
          </label>
          <label className="pe-field-label">Discounted Quantity
            <span className="pe-field-sub">Number of items that receive the discount.</span>
            <NumberInput min={1} value={String(form.discountedQty)} onChange={v => setForm(f => ({ ...f, discountedQty: Number(v) || 1 }))} />
          </label>
          <label className="pe-field-label">Discount %
            <span className="pe-field-sub">100% = free.</span>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <NumberInput min={1} max={100} value={String(form.discountPercent)} onChange={v => setForm(f => ({ ...f, discountPercent: Number(v) || 50 }))} />
              <div className="qd-quick-btns">
                {[25, 50, 75, 100].map(n => <button key={n} type="button" className="btn btn-ghost btn-sm" onClick={() => setForm(f => ({ ...f, discountPercent: n }))}>{n}%</button>)}
              </div>
            </div>
          </label>
        </div>
  ...
  return (
    <div className="adm-section">
      <div className="adm-sec-head">
        ...
        <button className="btn" onClick={openCreate}><Icon name="plus" size={14} /> Create Quantity Deal</button>
      </div>
      ...
      <div className="adm-info-card card">
        ...
        <div>
          ...
          <p className="muted" style={{ fontSize: ".84rem", marginTop: 4 }}>Quantity Deals automatically apply a discount when a customer buys a certain number of items. No coupon code required.</p>
        </div>
      </div>

      {loading ? <div className="adm-loading"><Icon name="spinner" size={24} className="is-spinning" /></div>
        : deals.length === 0
          ? <EmptyState icon="tag" title="No Quantity Deals" message="Create quantity deals to automatically reward bulk buyers." action={{ label: "Create Quantity Deal", onClick: openCreate }} />
          : (
            <div className="card" style={{ overflow: "hidden" }}>
              <table className="adm-table">
                <thead><tr><th>Name</th><th>Product</th><th>Deal</th><th>Discount</th><th>Status</th><th></th></tr></thead>
                <tbody>
                  {deals.map(d => (
                    <tr key={d.id}>
                      <td><strong>{d.name}</strong></td>
                      <td>{d.productName ?? d.productId}</td>
                      <td>Buy {d.buyQty}, get {d.discountedQty} at discount</td>
                      <td>{d.discountPercent}%</td>
                      <td><span className={`badge ${d.status === "enabled" ? "badge-green" : "badge-gray"}`}>{d.status}</span></td>
                      <td>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button className="btn btn-ghost btn-sm" onClick={() => openEdit(d)}><Icon name="settings" size={13} /></button>
                          <button className="btn btn-ghost btn-sm btn-danger-icon" onClick={() => remove(d.id)}><Icon name="close" size={13} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
