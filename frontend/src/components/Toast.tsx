      ...
      <div className="toast-stack" aria-live="polite" aria-atomic="false">
        {items.map((t) => (
          <div key={t.id} className={`toast ${t.tone}`} role="status">
            <Icon name={t.tone === "success" ? "check" : t.tone === "error" ? "close" : "bell"} size={16} />
            <span>{t.text}</span>
            <button className="t-x" onClick={() => remove(t.id)} aria-label="Dismiss"><Icon name="close" size={13} /></button>
          </div>
        ))}
      </div>
      <style>{`
...
export const useToast = (): ToastCtx => {
  ...
  if (!c) return {
    ...
    success: (t) => console.warn("[toast]", t),
    error: (t) => console.warn("[toast]", t),
    info: (t) => console.warn("[toast]", t),
  };
  return c;
};
