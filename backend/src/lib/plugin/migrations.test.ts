...
sqlite.run(
  `CREATE TABLE __plugin_migrations (plugin_id TEXT NOT NULL, idx INTEGER NOT NULL, applied_at INTEGER NOT NULL, PRIMARY KEY (plugin_id, idx))`,
);
...
const after = sqlite
  .query(`SELECT idx FROM __plugin_migrations WHERE plugin_id = 'plug-b' ORDER BY idx`)
  .all() as { idx: number }[];
