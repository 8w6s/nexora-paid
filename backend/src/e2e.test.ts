...
console.log("\n[E2E] checkout (login required)");
const noauth = await call("POST", "/api/checkout", { items: [{ productId: "prod-1", qty: 1 }] });
ok("checkout without auth → 400 (EMAIL_REQUIRED)", noauth.status === 400 && noauth.data.code === "EMAIL_REQUIRED");
...
ok("has LTC address", typeof co.data.ltcAddress === "string" && co.data.ltcAddress.length > 10, co.data.ltcAddress);
...
console.log("\n[E2E] simulate payment + idempotent delivery");
...
ok("delivered keys visible to owner", Array.isArray(detail.data.deliveredKeys) && detail.data.deliveredKeys.length === 2);

console.log("\n[E2E] admin");
const adminLogin = await call("POST", "/api/auth/login", { email: "admin@nexora.local", password: "admin12345" }, "a");
ok("admin login role=admin", adminLogin.data.role === "admin");
const stats = await call("GET", "/api/admin/stats", undefined, "a");
ok("admin stats revenue reflects paid order", stats.data.revenueUsd > 0, `$${stats.data.revenueUsd}`);
...
ok("admin can filter paid orders", Array.isArray(adminOrders.data) && adminOrders.data.some((o: any) => o.id === orderId));

// cleanup
console.log("\n[E2E] cleanup");
const db = new Database("sqlite.db");
db.run("UPDATE product_keys SET status='available', order_id=NULL, reserved_at=NULL, delivered_at=NULL WHERE order_id=?", [orderId]);
