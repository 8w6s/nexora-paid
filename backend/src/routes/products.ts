...
import { products, productKeys, productVariants } from "../db/schema.ts";
...
async function stockMap(productIds: string[]): Promise<Record<string, number>> {
  ...
  const rows = await db
    ...
    .where(
      and(
        inArray(productKeys.productId, productIds),
...
export const productRoutes = new Elysia()
  ...
  .get("/api/products", async ({ query }) => {
    ...
    const order =
      sort === "price-asc" ? asc(products.priceUsd)
      : sort === "price-desc" ? desc(products.priceUsd)
      : sort === "best" ? desc(products.sold)
    ...
    const variantStockRows = productIds.length > 0
      ? await db.select({ variantId: productKeys.variantId, c: count() })
          .from(productKeys)
  ...
  .get("/api/products/:idOrSlug", async ({ params: { idOrSlug }, set }) => {
    ...
    const variantsList = await db.select().from(productVariants).where(eq(productVariants.productId, row.id));
    const variantStockRows = await db.select({ variantId: productKeys.variantId, c: count() })
      .from(productKeys)
