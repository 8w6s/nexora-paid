...
# Pin compose image to current release tag
...
# Pin the image version in a .env.version file that compose reads
cat > "$RELEASE_DIR/.env.version" <<EOF
# Auto-generated by fulfill-order.sh — pin to the release the customer paid for.
NEXORA_VERSION=$CURRENT_VERSION
NEXORA_IMAGE=ghcr.io/8w6s/nexora-backend
NEXORA_IMAGE_FRONTEND=ghcr.io/8w6s/nexora-frontend
NEXORA_UPDATER_VERSION=$CURRENT_VERSION
EOF
...
echo ""
echo "Next steps:"
echo "  1. Issue a GHCR PAT (read:packages) for this customer:"
echo "     GitHub → Settings → Developer settings → Personal access tokens → Fine-grained"
echo "     Scope: read:packages on 8w6s/nexora-* repos. Expiry: 1 year."
echo "  2. Add GHCR_TOKEN=ghp_... to the customer's .env (or email separately)"
echo "  3. Upload ZIP to a private link (Catbox, Google Drive, S3 presigned URL)"
echo "  4. Email customer: download link + GHCR token + 'follow README.md'"
echo "  5. Record in your sales tracker"
echo ""