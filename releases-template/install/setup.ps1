...
if (Test-Path $EnvFile) {
    ...
    # Admin is created interactively via the /setup wizard on first boot.
    # Don't seed ADMIN_EMAIL/ADMIN_PASSWORD into .env — the backend would
    # otherwise bootstrap an account and skip the wizard entirely.
    @(
...
Write-Host '  First-run setup wizard:'
Write-Host '              ' -NoNewline; Write-Host "$PublicOrigin/setup" -ForegroundColor Green
Write-Host '  (create your admin account, paste your Litecoin xpub, done)' -ForegroundColor DarkGray
Write-Host ''
